...
function getXfn($type, $values, $count, $columns, $profile, $_language, $languages) {
  ...
  if ($_language->get('xfn_'.$type.'_i') != 'xfn_'.$type.'_i') {
    ...
    $xfn .= '  <td>'.$_language->get('xfn_'.$type).'</td>';
  }
  ...
  if ($type == 'delete_item') {
    ...
    if (in_array($type, array('urlify'))) {
      ...
      $xfn .= '          <option value="" '. ((isset($values['value']) && $values['value'] == $val) ? 'selected' : '') .'>'. $_language->get('text_urlify_default') .'</option>';
      $xfn .= '        </optgroup>';
      $xfn .= '        <optgroup label="'. $_language->get('text_urlify_ascii') .'">';
      foreach ($languages as $lang) {
      $xfn .= '          <option value="'.substr($lang['code'], 0, 2).'" '. ((isset($values['value']) && $values['value'] == $lang['code']) ? 'selected' : '') .'>'. $_language->get('text_translit') .' '. $lang['name'] .'</option>';
      }
      $xfn .= '        </optgroup>';
      $xfn .= '    </select>&nbsp;';
    }
    ...
    <tr class="nodrag nodrop">
      <td colspan="4" style="text-align:center" class="form-inline">
        <select class="form-control_ extra_func_select" id="extra_func_select" style="">
          ...
          <optgroup label="<?php echo $_language->get('xfn_group_'.$groupName); ?>">
            <?php foreach ($group as $funcName) { ?>
              <option value="<?php echo $funcName; ?>"><?php echo $_language->get('xfn_'.$funcName); ?></option>
            <?php } ?>
          <?php } ?>
          </optgroup>
        </select>

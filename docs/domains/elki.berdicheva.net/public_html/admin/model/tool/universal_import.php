...
class ModelToolUniversalImport extends Model {
  ...
  public function process_order_status_update($config, $line) {
    ...
    */
    ...
    if ($item_id) {
      $data['row_msg'] = '<a target="_blank" href="'.$this->url->link('sale/order/info', 'order_id='.$item_id.'&' . $this->token, 'SSL').'">'.$this->language->get('text_type_order').' #'.$item_id.'</a>';
    }
  ...
  */
  ...
  protected function manufacturerHandler(&$config) {
    ...
    if (is_numeric($config['columns']['manufacturer_id'])) {
      ...
      if ($this->simulation) {
        ...
        return $config['columns']['manufacturer_id'];
      }
    }
  ...
  protected function categoryHandler($field, &$config) {
    ...
    foreach ((array) $config['columns'][$field] as $categories) {
      ...
      if (is_string($categories) && !empty($config['col_binding'][htmlentities($categories, ENT_QUOTES, 'UTF-8')])) {
       if ($this->simulation) {
          $query = $this->db->query("SELECT name, category_id FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int) $config['col_binding'][htmlentities($categories, ENT_QUOTES, 'UTF-8')] . "'")->row;
          ...
          $values[] = $config['col_binding'][htmlentities($categories, ENT_QUOTES, 'UTF-8')];
        }
        ...
        foreach ($extra_funcs as $func_type => &$func) {
          if ($func['field'] !== '' && isset($line[$func['field']])) {
            ...
            if ($func_type == 'add') {
              ...
              $line[$target] = round($init_value, (int)$value);
            ...
            // String
              ...
              if (!empty($func['ascii'])) {
                ...
                $init_value = URLify::downcode($init_value, $func['ascii']);
              }
              $line[$target] = trim(mb_ereg_replace('--+', '-', str_replace(' ', '-', mb_strtolower($init_value))), '-');
      ...
      */
      ...
      for ($row = 1; $row <= $highestRow; $row++) {
        ...
        foreach ((array) $this->request->post['columns'][$cat_field] as $cat) {
          if (isset($row[$cat]) && !isset($rows[$row[$cat]])) {
            $rows[$row[$cat]] = isset($profile['col_binding'][$row[$cat]]) ? $profile['col_binding'][$row[$cat]] : '';
          }
        }
      }
    }
